...
std::ostream &Control_output::to_stream_data(
...
std::ostream &Control_output::to_stream_sc(
    std::ostream &os, CompoMe::Serialization_context_export &p_ctx) const {
  os << ",components:{";
...
std::ostream &Control_output::to_stream_provide(
    std::ostream &os, CompoMe::Serialization_context_export &p_ctx) const {
  os << ",provide:{";
...
/////////////////////////////////////////////////////////////////////////////
std::ostream &
Control_output::to_stream(std::ostream &os,
...
std::istream &Control_output::from_stream_provide(
  ...
  do {
    ...
    default:
      std::cerr << "wrong attribute: \"" << args
                << "\" not in data Control_output";
      throw "wrong attribute: \"" + args + "\" not in provide Control_output";
      break;
    }
...
std::istream &
Control_output::from_stream_data(std::istream &is,
                                 CompoMe::Serialization_context_import &p_ctx) {
  char l_c = is.get();
  if (l_c != '{') {
    std::cerr << "Wrong start: '" << l_c << "' != '{'";
  ...
  do {
    ...
    default:
      std::cerr << "wrong attribute: \"" << args
                << "\" not in data Control_output";
      throw "wrong attribute: \"" + args + "\" not in data Control_output";
      break;
    }
...
std::istream &
Control_output::from_stream_sc(std::istream &is,
                               CompoMe::Serialization_context_import &p_ctx) {
  char l_c = is.get();
  if (l_c != '{') {
    std::cerr << "Wrong start: '" << l_c << "' != '{'";
  ...
  do {
    ...
    default:
      ...
      throw "wrong attribute: \"" + args +
          "\" not in sub components Control_output";
      break;
    }
...
std::istream &
Control_output::from_stream(std::istream &is,
                            CompoMe::Serialization_context_import &p_ctx) {
  Control_output l_reset;
  *this = l_reset;
  ...
  do {
    ...
    case str2int("type"): {
      ...
      if (t.first != "Control_output") {
        throw "Wrong Type: "; // + "Control_output" + " != " + t.first ;
      }
      break;
    }
    ...
    default: {
      std::cerr << "wrong attribute: \"" << args << "\" not in Control_output";
      throw "wrong attribute: \"" + args + "\" not in Control_output";
      break;
    }
    }
